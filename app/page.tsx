...
export default function Home() {
  ...
  const handleCopyQuote = () => {
    ...
    let text: string
    if (isConsumer) {
      const locationLine = form.projectLocation || form.clientAddress
      text = `YOUR PROJECT ESTIMATE\n${form.jobDescription}${locationLine ? `\n${locationLine}` : ''}\n\n${lines}\n\nSubtotal: $${activeTotals?.subtotal?.toLocaleString()}\nTax: $${activeTotals?.tax?.toLocaleString()}\nTOTAL ESTIMATE: $${activeTotals?.total?.toLocaleString()}\n\nGenerated by SnapBid — snapbid.app`
    } else {
      text = `QUOTE #${quote.quoteNumber}${tierLabel}\nClient: ${form.clientName}\nAddress: ${form.clientAddress}${form.clientEmail ? `\nEmail: ${form.clientEmail}` : ''}\n\n${lines}\n\nSubtotal: $${activeTotals?.subtotal?.toLocaleString()}\nTax: $${activeTotals?.tax?.toLocaleString()}\nTOTAL: $${activeTotals?.total?.toLocaleString()}`
    }
  ...
  const handleEmailQuote = () => {
    ...
    if (isConsumer) {
      ...
      const locationLine = form.projectLocation || form.clientAddress
      subject = encodeURIComponent(`Your SnapBid Cost Estimate`)
      body = encodeURIComponent(
`Hi${form.clientName ? ` ${form.clientName}` : ''},

Here's your home project cost estimate from SnapBid.
...
Generated by SnapBid — snapbid.app`
      ...
      const exclList = quote.exclusions && quote.exclusions.length > 0
        ...
        : ''
      subject = encodeURIComponent(`Quote from ${biz} — ${quote.quoteNumber}`)
      body = encodeURIComponent(
`Hi ${form.clientName},
...
${biz}`
  ...
  const handleWhatsAppShare = () => {
    ...
    let text: string
    if (isConsumer) {
      text = `Here's my home project estimate from SnapBid:\n${form.jobDescription}\nTotal estimate: $${activeTotals?.total?.toLocaleString()}\nsnapbid.app`
    } else {
      const biz = profile?.businessName || form.businessName
      const tierLabel = quote.tiered ? ` (${selectedTier.charAt(0).toUpperCase() + selectedTier.slice(1)} option)` : ''
      const lineList = activeItems.map((item: any) =>
          ...
          <div className="max-w-3xl mx-auto">
                ...
                if (isConsumer) {
                  return (
                    <div className="flex items-center gap-3 text-xs flex-wrap">
                      <span className="text-gray-500">{history.length} saved estimate{history.length !== 1 ? 's' : ''}</span>
                    </div>
                  )
                }
            ...
            {!historyLoading && history.length > 0 && (
              <div className="flex flex-col sm:flex-row gap-2 mb-4">
                ...
                {/* Status filter chips — contractors only */}
                {profile?.businessName && (
                    ...
                    <div key={q.id} className={`bg-[#faf8f5] rounded-2xl border shadow-sm transition-all duration-200 ${isExpanded ? 'border-[#991b1b]/30 shadow-amber-50' : 'border-gray-100 hover:border-gray-200'}`}>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              {/* Status badges — contractors only */}
                              {profile?.businessName && q.status === 'won' && (
                              ...
                              {profile?.businessName && q.status === 'lost' && (
                              ...
                              {profile?.businessName && (!q.status || q.status === 'pending') && (
                      ...
                      {isExpanded && (
                        <div className="border-t border-gray-100 px-5 pb-5">
                          ...
                          {/* Notes — contractors only */}
                          {q.notes && profile?.businessName && (
                          ...
                          {/* Quote status tracking — contractors only */}
                          {profile?.businessName && (
                            ...
                            >
                              {reuseFlash === q.id ? (
                                ...
                                <>
                                  ...
                                  {profile?.businessName ? 'Reuse Quote' : 'Get Similar Estimate'}
                                </>
                              )}
                            </button>
                            <button
          ...
          <div className="animate-fade-in-up max-w-3xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-lg font-semibold text-gray-900">Your Project Estimate</h2>
            ...
            <div className="bg-[#faf8f5] rounded-2xl border border-gray-100 overflow-hidden shadow-md mb-4">
              ...
              {profile?.businessName ? (
                ...
                /* Consumer header — SnapBid branding, no quote number, no expiry */
                <div className="bg-[#991b1b] px-5 sm:px-8 py-5 sm:py-6 flex justify-between items-start gap-4">
                  <div>
                    <h3 className="text-white font-bold text-2xl tracking-tight">SnapBid</h3>
                    <p className="text-red-200 text-sm mt-1 font-medium">Your Project Estimate</p>
                  </div>
                  <div className="text-right">
                    <p className="text-red-200 text-xs">{new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                  </div>
                </div>
              )}
              ...
              <div className="px-5 sm:px-8 py-5 sm:py-6 space-y-6">
                ...
                {/* Scope of Work / What the job includes */}
                {quote.scopeOfWork && (
                  <div className="bg-gray-50 rounded-xl p-4">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">
                      {profile?.businessName ? 'Scope of Work' : 'What the job includes'}
                    </p>
                ...
                {quote.notes && profile?.businessName && (
                  <div className="bg-amber-50 border border-amber-100 rounded-xl p-4">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <button onClick={handleEmailQuote}
                ...
                {emailed ? (
                  ...
                  <>
                    ...
                    {profile?.businessName ? 'Email Quote' : 'Email Estimate'}
                    {form.clientEmail && (
              ...
              <button onClick={handleWhatsAppShare}
                ...
                title={profile?.businessName ? 'Share quote via WhatsApp' : 'Share estimate via WhatsApp'}
              ...
              <button onClick={handleCopyQuote}
                aria-label={profile?.businessName ? 'Copy quote to clipboard' : 'Copy estimate to clipboard'}
                ...
                {copied ? (
                  ...
                  <>
                    ...
                    {profile?.businessName ? 'Copy Quote' : 'Copy Estimate'}
                  </>
                )}
              </button>
              <button onClick={() => { setQuote(null); setGuestEmail(''); setGuestEmailSaved(false) }}
