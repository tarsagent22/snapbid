...
- Notes: ${isHomeownerMode ? "Leave this empty string '' — do not include payment terms, deposit requirements, or validity dates in homeowner estimates." : `payment terms, quote validity${yearsInBusiness ? `, mention ${yearsInBusiness} years of experience (exact number)` : ''}. Professional, 2-3 sentences max.${notesTemplate ? ` Always end notes with this contractor's custom message: "${notesTemplate}"` : ''}`}
